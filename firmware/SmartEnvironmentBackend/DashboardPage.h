...
<div class="shell">
  ...
  <main class="grid">
    ...
    <section class="card wide"><div class="title">Cảnh báo hệ thống</div><ul id="alerts"><li>Đang chờ dữ liệu cảm biến.</li></ul></section>
...
function render(d){const s=d.sensors||{},a=d.alerts||{};$('temperature').textContent=Number(d.temperature).toFixed(1)+' °C';$('light').textContent=String(d.lightLevel)+' / 1000';$('temperature-status').textContent=d.temperatureStatus||d.status?.temperature||'--';$('light-environment').textContent=d.lightEnvironment||d.status?.environment||'--';$('mode').textContent=d.mode;$('led').textContent=d.lightStatus?'ON':'OFF';$('source').textContent='Nguồn: '+(d.dataSource||'REAL');badge('temperature-online',s.temperature?.online!==false,'LM35');badge('light-online',s.light?.online!==false,'LDR');active('[data-mode]',d.mode);active('[data-light]',d.lightStatus?'ON':'OFF');const items=[];if(a.intruderDetected)items.push('CẢNH BÁO: Có người lạ vào nhà!');if(a.temperatureHigh)items.push('Nhiệt độ vượt ngưỡng.');if(a.lowLight)items.push('Ánh sáng dưới ngưỡng.');$('alerts').innerHTML=(items.length?items:['Hệ thống chưa phát hiện cảnh báo.']).map(x=>'<li>'+x+'</li>').join('');$('connection').textContent='ESP32 ONLINE';$('connection').className='badge ok';$('error').textContent=''}
